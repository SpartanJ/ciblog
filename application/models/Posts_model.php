...
class Posts_model extends CI_Model
{
	...
	{
		$this->db->query("UPDATE posts SET post_title = ?, post_body = ?, post_category = ?, post_draft = ?, post_slug = ?, post_updated = NOW() WHERE post_id = ?", array( $data['title'], $data['body'], $data['category'], $data['draft'], $slug, $data['post_id'] ) );
	}
	
	function get_drafts()
	{
		return $this->db->query("SELECT * FROM posts INNER JOIN categories ON post_category = cat_id WHERE post_draft = 1 ORDER BY post_created DESC")->result_array();
	}
	...
	function get_published( $category = NULL )
	{
		...
		return $this->db->query("SELECT * FROM posts INNER JOIN categories ON post_category = cat_id WHERE post_draft = 0 " . $and . " ORDER BY post_created DESC", array( $category ) )->result_array();
	}
	...
	function get_published_by_category_key( $cat_key = NULL )
	{
		...
		return $this->db->query("SELECT * FROM posts INNER JOIN categories ON post_category = cat_id WHERE post_draft = 0 " . $and . " ORDER BY post_created DESC", array( $cat_key ) )->result_array();
	}
